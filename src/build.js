...
const write = (pathName) => {
    ...
    fs.writeFileSync(
        ...
        fs
            ...
            .replace('setSelectionRa./confignge: function () {', '$& if (!window) return null;')
    ...
    fs.writeFileSync(
        ...
        fs
            .readFileSync(path.join(__dirname, pathName, 'lib/client/worker-hammerhead.js'), 'utf8')
            .replace(
    ...
    fs.writeFileSync(
        ...
        fs
            .readFileSync(path.join(__dirname, pathName, 'lib/client/transport-worker.js'), 'utf8')
            .replace(
                'proxyLocation.port.toString()',
                'proxyLocation.port?.toString() || (proxyLocation.protocol === "https:" ? 443 : 80)'
            )
...
if (fs.existsSync('../node_modules/testcafe-hammerhead')) {
    write('../node_modules/testcafe-hammerhead');
} else {
    write('../../../node_modules/testcafe-hammerhead');
}
